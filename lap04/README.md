# Dự án Thực hành Xử lý Ảnh Số

## Giới thiệu

Đây là bộ bài tập thực hành xử lý ảnh số. Mỗi lab tương ứng với một chủ đề cụ thể, áp dụng các kỹ thuật xử lý ảnh phổ biến bằng Python và thư viện OpenCV. Các bài lab giúp nắm vững từ thao tác cơ bản đến các kỹ thuật nâng cao.

### Mục tiêu

- Thực hiện phân vùng ảnh
- Thay đổi (biến đổi) đối tượng trong ảnh
- Áp dụng các kỹ thuật xử lý ảnh cơ bản: dịch chuyển, quay, co giãn, phản chiếu

### Các kỹ thuật sử dụng

- Dùng `PIL.Image` và `NumPy` để xử lý ảnh dưới dạng ma trận
- Thao tác affine transform (dịch, xoay, co giãn)
- Tách đối tượng ra khỏi ảnh nền bằng phân ngưỡng và lọc nhị phân
- Lưu ảnh kết quả bằng `imageio.imsave`

## Hướng dẫn sử dụng

1. Cài đặt thư viện cần thiết:

```bash
pip install -r requirements.txt
```

2. Mở từng notebook để chạy:

```bash
jupyter notebook Lap01_207ct09941.ipynb
```

## Yêu cầu hệ thống

- Python >= 3.7
- Jupyter Notebook
- OpenCV
- NumPy
- Matplotlib

## Giấy phép

Dự án phục vụ mục đích học tập cá nhân, phi thương mại.

## ✍️ Tác giả

- MSSV: 207CT09941
- Họ tên: Phạm Tuấn Anh
